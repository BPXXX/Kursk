...
void get_files(const fastring& dir, std::vector<fastring> &files, const fastring& ext /*= "*.*"*/, bool res /*= false*/)
{
	//文件句柄 
	intptr_t  hFile = 0;
	//文件信息 
	...
	fastring p = dir;
	hFile = _findfirst((dir + ext).c_str(), &fileinfo);
	if (hFile != -1)
	{
		do
		{
			...
			{
				if (res && strcmp(fileinfo.name, ".") != 0 && strcmp(fileinfo.name, "..") != 0)
				{
					get_files(dir + "/" + fileinfo.name, files, ext, res);
				}
			}
			else
			{
