...
MysqlQueryResult::MysqlQueryResult(MYSQL_RES *res, uint64_t row_count, uint32_t field_count)
...
uint32_t MysqlQueryResult::length(int n)
{
    return _mysql_res->lengths[n];
}


uint32_t MysqlQueryResult::get_data(int n, char *data)
{
    memcpy(data, _row[n], length(n));
    return length(n);
}

int32_t MysqlQueryResult::get_int32(int n)
{
    return _row[n] ? strtol(_row[n], 0, 10) : 0;
}

int64_t MysqlQueryResult::get_int64(int n)
{
    return _row[n] ? strtol(_row[n], 0, 10) : 0;
}
